...
If a message is nacked, a failure strategy is applied. Refer to `consumer.queue.failure-strategy` in <<extension-incoming-configuration-reference>>. The default strategy is set to `ignore` and move on to next message. Following are the strategies supported by Quarkus Solace Messaging Connector extension.
